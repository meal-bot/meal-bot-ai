...
def build_user_prompt(query: str, candidates: list[dict]) -> str:
    """사용자 질의와 후보 목록을 받아 LLM에 보낼 user message 문자열을 생성한다.

    Args:
        query: 사용자 자유 질의. 빈 문자열도 허용.
        candidates: hybrid top-N 후보 dict 리스트. 빈 리스트도 허용.

    Returns:
        구조화된 user prompt 문자열. candidates는 compact JSON으로 직렬화한다.
    """
