"""
v4-lite 평가 스크립트.
v1 semantic search + LLM rerank.
golden_set 기반 Precision@K, NDCG@K, MRR 계산.
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).parent.parent / "artifacts" / "eval_v4_lite_results.json",
    )
...
def idcg_at_k(num_relevant: int, k: int) -> float:
    return sum(1.0 / math.log2(rank + 2) for rank in range(min(num_relevant, k)))
...
def main() -> None:
    ...
    for qid, data in golden.items():
        ...
        try:
            ...
            print(f"[ERROR] query_id={qid} 실패: {e}")
            raise
