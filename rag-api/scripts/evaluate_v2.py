"""
v2 쿼리 재작성 평가 스크립트.
golden_set_v1_final.csv 기반으로 rewrite → search → Precision@K, NDCG@K, MRR 계산.
"""
...
from scripts.query_rewrite import rewrite
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).parent.parent / "artifacts" / "eval_v2_results.json",
    )
...
def main() -> None:
    ...
    for qid, data in golden.items():
        ...
        rewrite_result = rewrite(query)
        rewritten_query = rewrite_result["rewritten_query"]

        print(f"[{qid}] {query} → {rewritten_query}")

        try:
            hits = search(rewritten_query, top_k=TOP_K)
