...
[purpose 매핑]
- "가볍게"/"간단한"/"다이어트" → light
- "단백질"/"고기"/"헬스"/"운동 후"/"근육"/"근육 키우기"/"근육 위주" → protein
- "든든한"/"푸짐한"/"포만감" → hearty
- "맛있는"/"특별한"/"기분 좋은" → tasty
...
USER_PROMPT_TEMPLATE = """현재 시각: {now_formatted} (KST)
사용자가 시간대를 직접 말하지 않았는데 "지금", "이 시간", "이따", "방금" 등 현재 시점을 가리키면, 아래 기준으로 현재 시각을 meal_times에 매핑한다:
- 06:00~10:59 → 아침
- 11:00~14:59 → 점심
- 18:00~21:59 → 저녁
- 22:00~05:59 → 야식
- 위 어디에도 안 들어가면 → 간식
사용자가 시간대를 명시적으로 말하면("내일 아침", "저녁에") 그쪽을 우선하고 현재 시각은 무시한다.
